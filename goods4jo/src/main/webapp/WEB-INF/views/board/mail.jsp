...
if (vo == null) {%>
			...
			<div class="agile_mail_grids">
				<div class="col-md-12 contact-left">
					...
					<form action="saveQna.sajo" method="post">
						<input type="hidden" name="mid" required="" value="<%=vo.getMid()%>">
						<input type="text" name="qtitle" placeholder="Your Title"
							required=""> <input type="text" name="qpassword"
							placeholder="Your Password" required=""> <select
							type="text" name="qtype" required="" class="qtype" id='qtype'
						...
						<textarea name="qcontent" placeholder="Your Content" required="">
						</textarea>
						<input type="submit" align="center" value="보내기">
					</form>
					<br /> <br />
	...
	<table border="1" class="blueTable" id="report">
		<colgroup>
			<col width="60">
			<col width="100">
			<col width="100">
		...
		<c:forEach items="${qnaList }" var="qna">
			...
			<tbody>
				<tr>
					...
					<td>${qna.qtype }</td>
					<td>${qna.mid }</td>
					<td><fmt:parseDate value="${qna.qdate}" var="qdate"
							pattern="yyyy-MM-dd" /> <fmt:formatDate pattern="yyyy-MM-dd"
							value="${qdate }" /></td>
					<td>${qna.qtitle }<c:if test="${qna.qanswer == null}">
							<a href="getQna.sajo?qno=${qna.qno }"
								onclick="clickcr(this, 'not*g.list', 200012883, 1, event);">[답변달기]
							</a>
						</c:if></td>
				</tr>
				<tr>
					<td colspan="5">${qna.qanswer }</td>
				</tr>
			</tbody>
		</c:forEach>
	</table>
